...
f <- smoke ~  built_1980_to_1989 + built_1960_to_1969 + built_2010_to_later + 
              ...
              hhrace_black_only + hhrace_native_am_only + hh_white_only +
              mg_yes
...
plot_coefs(natl_coefs, title='National-level Coefficients and Std. Errors')
...
msa_t <- assess_msas(d)
# handle nulls
msa_t <- msa_t[-(which(msa_t$smsa=="99990")),]
```
...
TEST_MSA <- '73600'
GOOD_MSAS <- msa_t$smsa[msa_t$n>= MIN_MSA_OBS & msa_t$n_negative >= MIN_MSA_NEGATIVE_OBS]
write(GOOD_MSAS, 'data/msas.txt')
cat('Selected', length(msas), 'out of', nrow(msa_t), "MSAs")
cat('Dropped', (1-round(length(msas) / nrow(msa_t), 4)) * 100, '% of MSAs')
```
...
msa_coefs <- ldply(msas, estimate_msa, .progress='text')
...
# join msas to blockgroups.
j <- read.csv('data/msa_bg.csv', as.is=T)
j <- j[!is.na(j$smsa),]
j$bg_geoid <- as.character(j$bg_geoid)
j$smsa <- as.character(j$smsa)
...
compute_msa_score <- function(msa) { 
    d_m <- acs_bg[acs_bg$smsa==msa, ]
    coefs_m <- msa_coefs[msa_coefs$smsa == msa, ]
    d_m$msa_score  <-  compute_score(d_m, coefs_m, normalize=F) 
    d_m$msa_weight <- min(coefs_m$msa_weight)
    d_m$natl_weight <- min(coefs_m$natl_weight)
    subset(d_m, select=c('bg_geoid', 'smsa', 'msa_score', 'msa_weight', 'natl_weight'))
}

# msa_scores <- ldply(msas, compute_msa_score)


# compute national score
natl_score <- compute_score(acs_bg, natl_coefs, normalize=T)
output <- data.frame(smoke_alarm_risk=natl_score, bg_geoid=bg_geoid)
 
# plot a histogram of the computed scores
ggplot(output, aes(x=natl_score)) + 
