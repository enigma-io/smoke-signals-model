...
# initially run lasso regression with 10-folds cross validation to determine 
# ideal value of lambda
m <- as.matrix(d[,7:ncol(d)])

cvfit <- cv.glmnet(m, d$smoke, 
                  family = "binomial", type.measure = "class", alpha=0.1, nfolds=10)
l <- cvfit$lambda.min


# generate predictions and get list of coefficients
res <- predict(cvfit, s=l, type='coefficients')
res
res <- as.matrix(res)
res <- data.frame(variable=row.names(res), coefficient=as.numeric(res))
res <- res[order(res$coefficient, decreasing=T), ]
names(res) = c('variable', 'coefficient')
```
