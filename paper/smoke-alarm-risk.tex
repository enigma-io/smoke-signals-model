...
We estimate MSA-level coefficients by training an identical model on each MSA in the AHS. To select an appropriate list of MSAs to model, we explored the total number of respondents and respondents without smoke alarms per MSA (Figures 4 & 5). We chose to eliminate  all MSAs which have less than 60 total respondents and less than 10 respondents without smoke alarms.  This resulted in a list of 46 MSAs out of a possible 147.
...
AIC's for each model are normalized to a scale of 0.25 to 0.75. We use this value to determine how much we should weight the MSA-level score when computing a block group's overall risk (Figure 7).
